...
C_BAR_BG = (40, 40, 40)
C_BG_NIGHT_TOP = (10, 15, 30); C_BG_NIGHT_BOT = (40, 30, 60); C_MOON = (255, 255, 220)
C_ARENA_TOP = (60, 60, 70); C_ARENA_SIDE = (40, 40, 50)
...
def load_assets():
    def load_scale(path, size=None):
        try:
            ...
            s = pygame.Surface(size if size else (50,50)); s.fill((255, 0, 255)); return s
    ...
    if not ASSETS["menu_frames"]:
        f = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT)); f.fill(C_BG_DARK); ASSETS["menu_frames"].append(f)
    ...
    ASSETS["hero_walk"] = pygame.image.load("ASSETS/hero_ow_walk.png").convert_alpha() if os.path.exists("ASSETS/hero_ow_walk.png") else pygame.Surface((40,40))
    ...
    try:
        ...
        ASSETS["Slime_battle"] = frames_battle; ASSETS["Slime_ow"] = frames_ow
    except: pass
    ...
    try:
        ...
        ASSETS["Golem_battle"] = frames_battle; ASSETS["Golem_ow"] = frames_ow
    except: pass
...
class PlayerStats:
    ...
    def reset(self):
        ...
        self.day = 1
        self.water_current = 20; self.water_max = 20
    ...
    def add_xp(self, amount):
        ...
        while self.xp >= self.xp_next:
            ...
            self.max_hp += 50; self.max_mana += 20; self.max_stamina += 10
            self.hp = self.max_hp; self.mana = self.max_mana; self.stamina = self.max_stamina
            leveled_up = True
        return leveled_up
    ...
    def load_from_dict(self, data):
        ...
        self.level = data["level"]; self.xp = data["xp"]; self.xp_next = data["xp_next"]
        self.gold = data["gold"]; self.max_hp = data["max_hp"]; self.hp = data["hp"]
        self.max_mana = data["max_mana"]; self.mana = data["mana"]
        self.stamina = data["stamina"]; self.max_stamina = data["max_stamina"]
        self.hunger = data.get("hunger", 100); self.day = data["day"]
        ...
        for k, v in saved_inv.items(): self.inventory[k] = v
...
class SoundSynth:
    def __init__(self):
        self.sounds = {}; self.music_vol = 0.5; self.sfx_vol = 0.5
    ...
    def generate_sounds(self):
        ...
        self.set_sfx_volume(self.sfx_vol); self.set_music_volume(self.music_vol)
    def set_music_volume(self, val):
        self.music_vol = max(0.0, min(1.0, val)); pygame.mixer.music.set_volume(self.music_vol)
    def set_sfx_volume(self, val):
        self.sfx_vol = max(0.0, min(1.0, val))
        for s in self.sounds.values(): s.set_volume(self.sfx_vol)
    def make_noise(self, duration, decay=True, pitch_shift=1.0, volume=0.5):
        sample_rate = 44100; n_samples = int(sample_rate * duration)
        buf = array.array('h', [0] * n_samples)
        for i in range(n_samples):
            val = random.randint(-32000, 32000)
            if decay: val = int(val * (1 - (i / n_samples)))
    ...
    def make_tone(self, duration, freq, slide=None, tremolo=False):
        ...
        for i in range(n_samples):
            ...
            if tremolo: val *= (0.5 + 0.5 * math.sin(2 * math.pi * 10 * t))
            buf[i] = int(val * 0.3)
        return pygame.mixer.Sound(buffer=buf)
    def play(self, name):
...
def draw_text_shadow(surface, text, font, x, y, color=C_TEXT_WHITE, center=False):
    ...
    if center:
        ...
        surface.blit(s_surf, rect_s); surface.blit(t_surf, rect_t)
    else:
        surface.blit(s_surf, (x+2, y+2)); surface.blit(t_surf, (x, y))
    ...
    def update(self, events):
        ...
        for event in events:
            ...
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and self.is_hovered:
                AUDIO.play("select"); clicked = True
        return clicked

# --- ENTITAS & PARTIKEL ---
class Particle(pygame.sprite.Sprite):
    def __init__(self, x, y, color, size, life=30):
        super().__init__(); self.image = pygame.Surface((size, size), pygame.SRCALPHA)
        pygame.draw.circle(self.image, color, (size//2, size//2), size//2)
        self.rect = self.image.get_rect(center=(x, y))
        self.vx = random.uniform(-3, 3); self.vy = random.uniform(-3, 3); self.life = life; self.orig_life = life
    def update(self):
        self.rect.x += self.vx; self.rect.y += self.vy; self.life -= 1
        if self.orig_life > 0: self.image.set_alpha(int((self.life / self.orig_life) * 255))
...
class MenuParticle(Particle):
    ...
    def reset(self):
        ...
        self.speed = random.uniform(0.5, 2.0); self.size = random.randint(2, 4)
        self.color = random.choice([(255, 215, 0), (255, 255, 200), (200, 200, 255)]); self.alpha = 0
    def update(self):
        self.y -= self.speed
        if self.alpha < 200 and self.y > SCREEN_HEIGHT - 100: self.alpha += 2
        if self.y < 0: self.reset()
    def draw(self, surface):
...
class ProjectileWithTrail(pygame.sprite.Sprite):
    def __init__(self, start_pos, target_pos, color, particle_group, type="fire"):
        super().__init__(); self.type = type; self.color = color; self.particle_group = particle_group
        self.image = pygame.Surface((30, 30), pygame.SRCALPHA)
        if self.type == "fire": pygame.draw.circle(self.image, color, (15, 15), 15); pygame.draw.circle(self.image, (255, 255, 0), (15, 15), 8) 
        else: pygame.draw.polygon(self.image, color, [(15,0), (5, 15), (12, 15), (10, 30), (25, 10), (18, 10)])
        self.rect = self.image.get_rect(center=start_pos); self.target = target_pos; self.speed = 18
        dx = target_pos[0] - start_pos[0]; dy = target_pos[1] - start_pos[1]; dist = math.hypot(dx, dy)
        if dist != 0: self.vx = (dx / dist) * self.speed; self.vy = (dy / dist) * self.speed
        else: self.vx, self.vy = 0, 0
    def update(self):
        self.rect.x += self.vx; self.rect.y += self.vy
        if random.random() < 0.8:
            size = random.randint(5, 10); p_color = self.color if self.type == "thunder" else (255, 100, 0)
            trail = Particle(self.rect.centerx, self.rect.centery, p_color, size, life=10); self.particle_group.add(trail) 
        if math.hypot(self.rect.centerx - self.target[0], self.rect.centery - self.target[1]) < 25: self.kill()

class FloatingText(pygame.sprite.Sprite):
    def __init__(self, x, y, text, color, is_crit=False):
        super().__init__()
        font = FONT_DMG if is_crit else FONT_BOLD; text_str = str(text) + "!" if is_crit else str(text)
        base_txt = font.render(text_str, True, color); outline = font.render(text_str, True, (0,0,0))
        w, h = base_txt.get_size(); scale = 1.5 if is_crit else 1.0
        ...
        self.rect = self.image.get_rect(center=(x, y)); self.timer = 60
        self.vy = -4 if is_crit else -2; self.vx = random.uniform(-1.5, 1.5); self.gravity = 0.15
    def update(self):
        self.rect.y += self.vy; self.rect.x += self.vx; self.vy += self.gravity; self.timer -= 1
        if self.timer < 15: self.image.set_alpha(int((self.timer/15)*255))
        if self.timer <= 0: self.kill()

# --- [PBO: PEWARISAN] KELAS ENTITY (PARENT) ---
class Entity(pygame.sprite.Sprite):
    def __init__(self, name, hp, max_mana, x, y):
        super().__init__(); self.name = name; self.max_hp = hp; self.hp = hp; self.max_mana = max_mana; self.mana = max_mana
        self.image = pygame.Surface((160, 160), pygame.SRCALPHA); self.image_to_draw = self.image 
        self.rect = self.image.get_rect(midbottom=(x, y)); self.start_pos = pygame.Vector2(x, y); self.is_dead = False
        self.anim_tick = 0; self.shake_timer = 0; self.flash_timer = 0; self.lunge_timer = 0; self.is_facing_right = True
    ...
    def take_damage(self, amount, type="phys", is_crit=False):
        ...
        color = (255, 50, 50); sound_to_play = "hit"
        if type == "magic": color = (200, 100, 255)
        if amount < 0: color = (50, 255, 100); amount = abs(amount); sound_to_play = None
        if sound_to_play: AUDIO.play(sound_to_play)
        self.shake_timer = 25 if is_crit else 10; self.flash_timer = 8 if is_crit else 5
        return FloatingText(self.rect.centerx, self.rect.top - 20, str(int(amount)), color, is_crit)
    
    def trigger_lunge(self): self.lunge_timer = 15 
    
    # [PBO: POLIMORFISME]
    def update(self):
        self.anim_tick += 0.05; offset_x = 0
        if self.lunge_timer > 0:
            self.lunge_timer -= 1
            if self.lunge_timer > 10: offset_x = 60 if self.is_facing_right else -60; 
            elif self.lunge_timer > 0: offset_x = -20 if self.is_facing_right else 20
        shake_off = pygame.Vector2(0,0)
        if self.shake_timer > 0: shake_off.x = random.randint(-5, 5); shake_off.y = random.randint(-3, 3); self.shake_timer -= 1
        ...
        if self.flash_timer > 0: self.flash_timer -= 1; flash_surf = self.image.copy(); flash_surf.fill((255, 255, 255, 200), special_flags=pygame.BLEND_RGBA_MULT); self.image_to_draw = flash_surf
        else: self.image_to_draw = self.image
        if self.hp <= 0 and not self.is_dead: self.is_dead = True

# --- [PBO: PEWARISAN] KELAS HERO (CHILD) ---
class Hero(Entity):
    def __init__(self, stats):
        super().__init__(f"Lv.{stats.level} {stats.player_name}", stats.max_hp, stats.max_mana, 250, 440)
        self.hp = stats.hp; self.mana = stats.mana 
    ...
    def regen_mana(self): self.mana = min(self.max_mana, self.mana + 15)
...
class Enemy(Entity):
    def __init__(self, level, enemy_type):
        ...
        spawn_y = 440
        if self.type == "Golem": spawn_y = 410 
        super().__init__(f"Lv.{level} {self.type}", base_hp, 0, 750, spawn_y)
        self.level = level; self.is_facing_right = False
        if self.type == "Slime" and ASSETS.get("Slime_battle"): self.image = ASSETS["Slime_battle"][0]
        elif self.type == "Golem" and ASSETS.get("Golem_battle"): self.image = pygame.transform.flip(ASSETS["Golem_battle"][0], True, False)
        else: self.redraw()
    
    # [PBO: POLIMORFISME]
    def redraw(self):
        self.image.fill((0,0,0,0)); cx, cy = 80, 80
        if self.type == "Slime": pygame.draw.circle(self.image, (50, 200, 50), (cx, cy), 40)
        elif self.type == "Golem": pygame.draw.rect(self.image, (100, 100, 100), (cx-40, cy-40, 80, 80))
        else: pygame.draw.circle(self.image, (255, 0, 0), (cx, cy), 40)
...
class FarmTile:
    def __init__(self, global_x, global_y, biome_type):
        self.global_x = global_x; self.global_y = global_y; self.biome = biome_type 
        self.state = "NATURAL"; self.crop_type = None; self.growth_stage = 0; self.growth_timer = 0; self.soil_decay_timer = 0
    ...
    def interact(self, tool):
        ...
        if self.crop_type and self.growth_stage == 3:
            # FIX: Menggunakan .get() untuk mencegah KeyError jika crop belum pernah dipanen
            ...
            name = self.crop_type; self.crop_type = None; self.growth_stage = 0; self.state = "TILLED"
            AUDIO.play("buy")
        ...
        if tool == "WATER" and self.biome == "WATER":
            GAME_STATS.water_current = GAME_STATS.water_max; AUDIO.play("water"); return True, "Water Refilled!"
        
        # Validasi Biome
        if self.biome == "WATER" or self.biome == "FOREST": return False, "Can't farm here!"
            ...
            if self.state == "NATURAL":
                if self.biome in ["GRASS", "DIRT"]:
                    if GAME_STATS.stamina >= 5: 
                        GAME_STATS.stamina -= 5; self.state = "TILLED"; self.soil_decay_timer = 0; AUDIO.play("hoe"); return True, "Soil Tilled!"
                    else: return False, "Out of Stamina!"
                else: return False, "Ground too hard!" 
            elif self.state == "TILLED": return False, "Already Tilled!"
            
        # Logika Tanam Bibit
        elif "SEED" in tool:
            if self.state == "NATURAL": return False, "Till soil first!"
            elif self.crop_type is not None: return False, "Crop already planted!"
            seed_name, crop_name = "", ""
            ...
            if seed_name and GAME_STATS.inventory.get(seed_name, 0) > 0:
                GAME_STATS.inventory[seed_name] -= 1; self.crop_type = crop_name; self.growth_stage = 0; self.growth_timer = 0; AUDIO.play("step"); return True, f"Planted {crop_name}!"
            else: AUDIO.play("error"); return False, "Out of Seeds!"
            ...
            if self.state == "TILLED" or (self.crop_type and self.state != "WATERED"):
                if GAME_STATS.water_current > 0:
                    if GAME_STATS.stamina >= 3: 
                        GAME_STATS.stamina -= 3; self.state = "WATERED"; GAME_STATS.water_current -= 1; self.soil_decay_timer = 0; AUDIO.play("water"); return True, "Watered!"
                    else: return False, "Out of Stamina!"
                else: return False, "Empty! Refill at water."
        return False, "" 
    ...
    def update(self):
        if self.crop_type and self.state == "WATERED":
            if self.growth_stage < 3:
                self.growth_timer += 1
                if self.growth_timer >= 600: self.growth_stage += 1; self.growth_timer = 0; self.state = "TILLED" 
        if self.state == "TILLED" and self.crop_type is None:
            self.soil_decay_timer += 1
            if self.soil_decay_timer >= 1200: self.state = "NATURAL"; self.soil_decay_timer = 0
        else: self.soil_decay_timer = 0
    ...
    def draw(self, surface, camera_x, camera_y):
        ...
        if self.crop_type:
            if ASSETS.get("crops") and self.crop_type in ASSETS["crops"]:
                ...
                size = (self.growth_stage + 1) * 6; col = (255, 255, 200)
                ...
                if self.growth_stage < 3: pygame.draw.circle(surface, (50, 200, 50), (cx, cy+5), size)
                else: pygame.draw.circle(surface, col, (cx, cy-5), 12)
...
class WorldSystem:
    def __init__(self):
        self.tiles = {}; self.seed = 999; self.generate_fixed_map()
    def generate_fixed_map(self):
        center_x = MAP_WIDTH // 2; center_y = MAP_HEIGHT // 2
        for y in range(MAP_HEIGHT):
            for x in range(MAP_WIDTH):
                biome = "GRASS"
                if x == 0 or x == MAP_WIDTH - 1 or y == 0 or y == MAP_HEIGHT - 1: biome = "WATER"
                elif abs(x - center_x) < 5 and abs(y - center_y) < 5: biome = "GRASS"
                else:
    ...
    def reset_world(self):
        for tile in self.tiles.values(): tile.state = "NATURAL"; tile.crop_type = None; tile.growth_stage = 0; tile.growth_timer = 0; tile.soil_decay_timer = 0
    def get_tile_at(self, global_x, global_y): return self.tiles.get((int(global_x), int(global_y)))
    def get_all_tiles(self): return self.tiles.values()
...
class PlayerOverworld(pygame.sprite.Sprite):
    def __init__(self, hotbar):
        super().__init__(); self.hotbar = hotbar
        self.global_x = (MAP_WIDTH // 2) * TILE_SIZE; self.global_y = ((MAP_HEIGHT // 2) * TILE_SIZE) + 150 
        self.rect = pygame.Rect(0, 0, 40, 40); self.speed = 5; self.facing_right = True; self.facing_dir = "DOWN"  
        self.sheet = ASSETS["hero_walk"]; self.frame_index = 0; self.animation_speed = 0.15; self.is_moving = False
        self.sheet_w, self.sheet_h = self.sheet.get_size(); self.frame_width = self.sheet_w // 6; self.frame_height = self.sheet_h // 8 
    ...
    def get_frame(self, col, row, flip=False):
        ...
        if flip: scaled_img = pygame.transform.flip(scaled_img, True, False)
        return scaled_img
    def update_animation(self):
        target_row = 0 
        if self.is_moving: target_row = 1
        self.frame_index += self.animation_speed
        if self.frame_index >= 6: self.frame_index = 0
        self.image = self.get_frame(int(self.frame_index), target_row, flip=not self.facing_right)
    def update(self, keys, obstacles):
        dx, dy = 0, 0; self.is_moving = False
        if keys[pygame.K_a]: dx = -self.speed; self.facing_right = False; self.facing_dir = "LEFT"; self.is_moving = True
        if keys[pygame.K_d]: dx = self.speed; self.facing_right = True; self.facing_dir = "RIGHT"; self.is_moving = True
        if keys[pygame.K_w]: dy = -self.speed; self.facing_dir = "UP"; self.is_moving = True
        if keys[pygame.K_s]: dy = self.speed; self.facing_dir = "DOWN"; self.is_moving = True
        if dx != 0 or dy != 0:
            next_x = self.global_x + dx; temp_rect = pygame.Rect(0, 0, 40, 40); temp_rect.center = (next_x, self.global_y); collision_x = False
            for obs in obstacles:
                if temp_rect.colliderect(obs): collision_x = True; break
            if 0 <= next_x <= (MAP_WIDTH * TILE_SIZE) - 40 and not collision_x: self.global_x = next_x
            next_y = self.global_y + dy; temp_rect.center = (self.global_x, next_y); collision_y = False
            for obs in obstacles:
                if temp_rect.colliderect(obs): collision_y = True; break
            if 0 <= next_y <= (MAP_HEIGHT * TILE_SIZE) - 40 and not collision_y: self.global_y = next_y
            if random.random() < 0.05: AUDIO.play("step")
        self.update_animation(); self.rect.center = (self.global_x, self.global_y - 10)

class OverworldEnemy(pygame.sprite.Sprite):
    def __init__(self, global_x, global_y, level, enemy_type):
        super().__init__(); self.global_x = global_x; self.global_y = global_y; self.level = level; self.type = enemy_type
        self.image = pygame.Surface((60, 60), pygame.SRCALPHA); self.rect = self.image.get_rect(center=(self.global_x, self.global_y))
        self.move_timer = 0; self.vx = random.choice([-1, 1]); self.vy = 0; self.facing_right = True; self.anim_frames = None; self.anim_idx = 0; self.anim_speed = 0.2
        if self.type == "Slime": self.anim_frames = ASSETS.get("Slime_ow")
        elif self.type == "Golem": self.anim_frames = ASSETS.get("Golem_ow")
    def update(self, safe_zone_rect):
        if self.anim_frames:
            self.anim_idx += self.anim_speed
            if self.anim_idx >= len(self.anim_frames): self.anim_idx = 0
        self.move_timer -= 1
        if self.move_timer <= 0: self.move_timer = random.randint(60, 180); self.vx = random.uniform(-2, 2); self.vy = random.uniform(-2, 2)
        if self.vx > 0: self.facing_right = True
        elif self.vx < 0: self.facing_right = False
        new_x = self.global_x + self.vx; new_y = self.global_y + self.vy; temp_rect = self.image.get_rect(center=(new_x, new_y))
        if safe_zone_rect.colliderect(temp_rect): self.vx *= -1; self.vy *= -1 
        else:
            if 50 < new_x < (MAP_WIDTH * TILE_SIZE) - 50: self.global_x = new_x
            else: self.vx *= -1
            if 50 < new_y < (MAP_HEIGHT * TILE_SIZE) - 50: self.global_y = new_y
            else: self.vy *= -1
        ...
        if self.anim_frames:
            ...
            cx, cy = 30, 30; sl_w = current_frame.get_width(); sl_h = current_frame.get_height(); self.image.blit(current_frame, (cx - sl_w//2, cy - sl_h//2))
            lvl_surf = FONT_UI.render(f"Lv{self.level}", True, C_ACCENT_GOLD); self.image.blit(lvl_surf, lvl_surf.get_rect(center=(cx, cy - 25)))
        else: self.image.fill((0,0,0,0)); pygame.draw.circle(self.image, (150, 0, 0), (30, 30), 18)

# --- MESIN PERTARUNGAN ---
class BattleScene:
    def __init__(self):
        self.active = False; self.won = False
        self.all_sprites = pygame.sprite.Group(); self.particles = pygame.sprite.Group(); self.texts = pygame.sprite.Group(); self.projectiles = pygame.sprite.Group()
    
    def start(self, enemy_level, enemy_type):
        self.active = True; self.won = False
        self.hero = Hero(GAME_STATS); self.enemy = Enemy(enemy_level, enemy_type)
        self.all_sprites.empty(); self.particles.empty(); self.texts.empty(); self.projectiles.empty()
        self.all_sprites.add(self.hero, self.enemy)
        self.turn = "PLAYER"; self.log_text = f"BATTLE START! Vs {self.enemy.name}"
        self.turn_timer = 0; self.temp_dmg = (0, False); self.temp_enemy_dmg = (0, False); self.screen_shake_intensity = 0
        AUDIO.play("battle_start")
    
    def deal_damage_w_effect(self, target, amount, type, is_crit=False):
        txt = target.take_damage(amount, type, is_crit); self.texts.add(txt)
        ...
        if is_crit: self.screen_shake_intensity = 15 
    
    def spawn_particles(self, x, y, color, count=15, speed_mult=1):
        for _ in range(count):
            p = Particle(x, y, color, random.randint(5, 10)); p.vx *= speed_mult; p.vy *= speed_mult; self.particles.add(p)
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            mx, my = event.pos
            for i in range(4):
                bx = 240 + (i * 150); rect = pygame.Rect(bx, 515, 140, 70)
                if rect.collidepoint(mx, my): action_idx = i; AUDIO.play("select"); break
        if action_idx != -1:
            cost, dmg, skill = 0, 0, ""
            lvl_bonus = GAME_STATS.level * 5
            if action_idx == 0: 
                cost, dmg, skill = 0, random.randint(30, 45) + lvl_bonus, "Slash"; self.hero.trigger_lunge(); AUDIO.play("slash")
            elif action_idx == 1: 
                cost = 30
                if self.hero.mana >= cost:
                    self.hero.mana -= cost; heal = random.randint(60, 100) + lvl_bonus
                    self.texts.add(self.hero.take_damage(-heal, "heal")); self.spawn_particles(self.hero.rect.centerx, self.hero.rect.centery, (50, 255, 100), 30)
                    AUDIO.play("heal"); self.log_text = "Hero recovered HP!"; self.end_player_turn()
                return 
            elif action_idx == 2: 
                cost, dmg, skill = 40, random.randint(70, 110) + lvl_bonus, "Fireball"
                if self.hero.mana >= cost and self.enemy: AUDIO.play("magic"); proj = ProjectileWithTrail(self.hero.rect.center, self.enemy.rect.center, (255, 100, 0), self.particles, "fire"); self.projectiles.add(proj)
            elif action_idx == 3: 
                cost, dmg, skill = 90, random.randint(150, 220) + lvl_bonus, "Thunder"
                if self.hero.mana >= cost and self.enemy: AUDIO.play("thunder"); proj = ProjectileWithTrail(self.hero.rect.center, self.enemy.rect.center, (100, 200, 255), self.particles, "thunder"); self.projectiles.add(proj)
            if skill and self.hero.mana >= cost:
                self.hero.mana -= cost; is_crit = random.random() < (0.2 + (GAME_STATS.level * 0.01)); dmg = int(dmg * 1.5) if is_crit else dmg
                if skill == "Slash": pygame.time.set_timer(pygame.USEREVENT + 2, 250, 1); self.temp_dmg = (dmg, is_crit)
                else: self.temp_dmg = (dmg, is_crit) 
                self.log_text = f"Hero used {skill}!"; self.end_player_turn()
            elif skill: self.log_text = "Not Enough Mana!"
    ...
    def end_player_turn(self): self.turn = "ENEMY_WAIT"; self.turn_timer = 60 
    def enemy_attack(self):
        if not self.enemy or self.enemy.is_dead: return
        self.enemy.trigger_lunge(); AUDIO.play("slash")
        delay = 200 
        if self.enemy.type in ["Golem", "Knight"]: delay = 350
        is_crit = random.random() < 0.2; dmg = random.randint(20, 30 + self.enemy.level * 6); dmg = int(dmg * 1.5) if is_crit else dmg
        pygame.time.set_timer(pygame.USEREVENT + 3, delay, 1); self.temp_enemy_dmg = (dmg, is_crit); self.log_text = f"{self.enemy.name} Attacks!"
        self.hero.regen_mana(); self.turn = "PLAYER"
    def update(self):
        self.all_sprites.update(); self.particles.update(); self.texts.update(); self.projectiles.update()
        shake_off = (0,0)
        if self.screen_shake_intensity > 0:
            shake_off = (random.randint(-self.screen_shake_intensity, self.screen_shake_intensity), random.randint(-self.screen_shake_intensity, self.screen_shake_intensity)); self.screen_shake_intensity -= 1
        self.shake_offset = shake_off
        if self.enemy:
            hits = pygame.sprite.spritecollide(self.enemy, self.projectiles, True)
            for proj in hits: dmg, is_crit = self.temp_dmg; self.deal_damage_w_effect(self.enemy, dmg, "magic", is_crit)
        if self.turn == "ENEMY_WAIT":
            self.turn_timer -= 1; 
            if self.turn_timer <= 0: self.enemy_attack()
            
        if self.enemy and self.enemy.is_dead and self.turn != "WIN_WAIT":
             xp = 50 + (self.enemy.level * 10); drops = {"Slime": "Slime Gel", "Golem": "Gold Nugget"}; drop_item = drops.get(self.enemy.type, "Slime Gel")
             if random.random() < 0.05: drop_item = "Dark Essence"
             ...
             if GAME_STATS.add_xp(xp): AUDIO.play("levelup"); msg = "LEVEL UP! " + msg
             if msg_quest: msg = "Quest Completed!"
             
             self.log_text = msg; self.turn = "WIN_WAIT"; self.win_timer = 120; self.won = True
             
        if self.hero.hp <= 0 and self.turn != "WIN_WAIT": self.log_text = "DEFEATED! Running away..."; self.win_timer = 120; self.turn = "WIN_WAIT"; self.won = False 
        if self.turn == "WIN_WAIT":
            self.win_timer -= 1
            if self.win_timer <= 0: 
                self.active = False
                if self.won: GAME_STATS.hp = self.hero.hp; GAME_STATS.mana = self.hero.mana
    def draw(self, surface):
        ...
        for entity in [self.hero, self.enemy]:
            if entity and not entity.is_dead:
                 s_x = entity.rect.centerx - 40 + offset[0]; s_y = entity.rect.bottom - 15 + offset[1]; pygame.draw.ellipse(surface, (0,0,0,100), (s_x, s_y, 80, 25))
        for sprite in self.all_sprites: img = getattr(sprite, 'image_to_draw', sprite.image); surface.blit(img, sprite.rect.move(offset))
        for p in self.particles: surface.blit(p.image, p.rect.move(offset))
        for p in self.projectiles: surface.blit(p.image, p.rect.move(offset))
        self.texts.draw(surface)
        pygame.draw.rect(surface, (30, 30, 40), (0, 500, SCREEN_WIDTH, 100)); pygame.draw.line(surface, C_ACCENT_GOLD, (0, 500), (SCREEN_WIDTH, 500), 3)
        bx = 240
        ...
        for name, cost, col in skills:
            ...
            pygame.draw.rect(surface, bg, (bx, 515, 140, 70), border_radius=8); pygame.draw.rect(surface, (70, 70, 80), (bx, 515, 140, 70), 2, border_radius=8)
            surface.blit(FONT_BOLD.render(name, True, col), (bx+10, 525)); surface.blit(FONT_UI.render(cost, True, (150,150,150)), (bx+10, 550)); bx += 150
        self.hero.draw_bar_premium(surface, 20, 540, self.hero.hp, self.hero.max_hp, (220, 60, 60), "HP"); self.hero.draw_bar_premium(surface, 20, 580, self.hero.mana, self.hero.max_mana, (60, 100, 220), "MP")
        if self.enemy and not self.enemy.is_dead: self.enemy.draw_bar_premium(surface, SCREEN_WIDTH-170, 540, self.enemy.hp, self.enemy.max_hp, (220, 60, 60), "HP")
        log_bg = pygame.Surface((600, 50), pygame.SRCALPHA); pygame.draw.rect(log_bg, (0,0,0,180), log_bg.get_rect(), border_radius=10); surface.blit(log_bg, (200, 10))
        txt = FONT_BOLD.render(self.log_text, True, C_ACCENT_GOLD); surface.blit(txt, txt.get_rect(center=(SCREEN_WIDTH//2, 35)))

# --- HOTBAR & UI ---
class Hotbar:
    def __init__(self):
        self.slots = ["HOE", "WATER", "SEED_GRAPE", "SEED_MUSHROOM", "POTION"]
        self.selected = 0; self.slot_size = 50
    def draw(self, surface, x_pos=None, y_pos=None):
        ...
        for i, slot in enumerate(self.slots):
            ...
            if i == self.selected: pygame.draw.rect(surface, C_ACCENT_GOLD, rect, 3, border_radius=8)
            else: pygame.draw.rect(surface, (80, 80, 80), rect, 1, border_radius=8)
            ...
            if slot == "WATER":
                ...
                if w_ratio > 0: pygame.draw.rect(surface, (50, 150, 255), (x+5, y+40, 40 * w_ratio, 5))
...
class MasterGame:
    def __init__(self):
        ...
        self.hotbar = Hotbar(); self.player = PlayerOverworld(self.hotbar)
        self.world = WorldSystem(); self.battle = BattleScene()
        self.enemies_ow = pygame.sprite.Group() 
        self.msg_text = ""; self.msg_timer = 0; self.shop_tab = "BUY"; self.shop_category = "SEEDS" 
        self.selected_item_name = None 
        self.game_time = 6 * 60; self.time_speed = 0.05; self.hunger_timer = 0
        ...
        center_x = (MAP_WIDTH // 2) * TILE_SIZE; center_y = (MAP_HEIGHT // 2) * TILE_SIZE
        self.house_rect = pygame.Rect(center_x - 125, center_y - 200, 200, 150)
        self.safe_zone_rect = self.house_rect.inflate(500, 500)
        self.fade_alpha = 0; self.fade_state = "IDLE"; self.pending_pass_out = False; self.black_screen_timer = 0
    ...
    def stop_menu_music(self): pygame.mixer.music.fadeout(500)
    ...
    def try_sleep(self):
        if math.hypot(self.player.global_x - self.house_rect.centerx, self.player.global_y - (self.house_rect.bottom - 40)) < 80: 
            if self.game_time >= 1080: self.trigger_day_transition(passed_out=False); return True
            else: self.show_msg("Too early to sleep!"); return True 
        return False

    def trigger_day_transition(self, passed_out=False):
        if self.fade_state == "IDLE": self.fade_state = "FADE_OUT"; self.pending_pass_out = passed_out; AUDIO.play("save") 

    def _perform_day_reset(self):
        GAME_STATS.day += 1; GAME_STATS.stamina = GAME_STATS.max_stamina
        if self.pending_pass_out: GAME_STATS.hp = max(1, GAME_STATS.hp // 2); GAME_STATS.gold = max(0, GAME_STATS.gold - 50); self.show_msg("Passed out! Lost energy & gold.")
        else: GAME_STATS.hp = min(GAME_STATS.max_hp, GAME_STATS.hp + 50); self.show_msg(f"Day {GAME_STATS.day} Started!")
        self.game_time = 6 * 60; self.player.global_x = (MAP_WIDTH // 2) * TILE_SIZE; self.player.global_y = ((MAP_HEIGHT // 2) * TILE_SIZE) + 150 
        self.spawn_enemies(20); AUDIO.play("heal")
    ...
    def try_consume_item(self):
        ...
        if item_name and GAME_STATS.inventory.get(item_name, 0) > 0:
            ...
            # --- [PERBAIKAN] LOGIKA KONSUMSI ITEM (CEK STATUS PENUH) ---
    ...
    def clean_hotbar(self):
        for i in range(len(self.hotbar.slots)):
            ...
            if slot_code in CODE_TO_NAME:
                ...
                if real_name in ["Hoe", "Watering Can"]: continue
                if GAME_STATS.inventory.get(real_name, 0) <= 0: self.hotbar.slots[i] = "EMPTY" 

    def spawn_enemies(self, count):
        self.enemies_ow.empty(); p_gx = self.player.global_x; p_gy = self.player.global_y; spawned_count = 0; attempts = 0
        while spawned_count < count and attempts < 1000:
            attempts += 1; gx = random.randint(2, MAP_WIDTH - 3) * TILE_SIZE; gy = random.randint(2, MAP_HEIGHT - 3) * TILE_SIZE
            ...
            if not self.safe_zone_rect.colliderect(enemy_test_rect) and math.hypot(gx - p_gx, gy - p_gy) > 400:
                ...
                if tile and tile.biome != "WATER":
                    min_level = max(1, GAME_STATS.level - 2); max_level = GAME_STATS.level + 2; level = random.randint(min_level, max_level); etype = random.choice(["Slime", "Golem"])
                    self.enemies_ow.add(OverworldEnemy(gx, gy, level, etype)); spawned_count += 1
    ...
    def start_new_game(self):
        ...
        self.player.global_x = (MAP_WIDTH // 2) * TILE_SIZE; self.player.global_y = ((MAP_HEIGHT // 2) * TILE_SIZE) + 150 
        self.spawn_enemies(20); self.game_time = 6 * 60; self.state = STATE_OVERWORLD

    def check_encounter(self):
        hits = pygame.sprite.spritecollide(self.player, self.enemies_ow, True, pygame.sprite.collide_rect_ratio(0.9))
        if hits:
            enemy_hit = hits[0]; self.state = STATE_BATTLE; self.battle.start(enemy_hit.level, enemy_hit.type)
            self.show_msg(f"Battle: Lv{enemy_hit.level} {self.battle.enemy.type}!"); AUDIO.play("battle_start")

    def show_msg(self, txt): 
        if txt: self.msg_text = txt; self.msg_timer = 60
    
    def get_tile_under_mouse(self, camera_x, camera_y):
        mx, my = get_corrected_mouse()
        world_mx = mx + camera_x; world_my = my + camera_y
        gx = int(world_mx // TILE_SIZE); gy = int(world_my // TILE_SIZE)
    ...
    def save_game(self):
        try:
            ...
            AUDIO.play("save"); self.show_msg(f"Saved as {filename}!")
        except Exception as e:
            print(e)
    ...
    def load_game(self, filename):
        if not os.path.exists(filename): self.show_msg("Save File Not Found!"); return
        try:
            ...
            self.stop_menu_music() 
            AUDIO.play("heal"); self.show_msg(f"Loaded {GAME_STATS.player_name}!"); self.state = STATE_OVERWORLD; self.spawn_enemies(20)
        except Exception as e:
            print(e)
            self.show_msg("Failed to Load!")

    def draw_time_filter(self):
        alpha = 0; hour = (self.game_time // 60) % 24
        if 18 <= hour < 20: alpha = 100 
        elif 20 <= hour or hour < 5: alpha = 180 
        elif 5 <= hour < 7: alpha = 80 
        if alpha > 0: overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT)); overlay.set_alpha(alpha); overlay.fill((10, 10, 30)); SCREEN.blit(overlay, (0,0))

    # --- FUNGSI MENGGAMBAR UI ---
    def draw_hud(self):
        draw_panel(SCREEN, pygame.Rect(10, 10, 240, 130), border=True)
        hour = int((self.game_time // 60) % 24); minute = int(self.game_time % 60)
        ...
        # Panel diperbesar (height 80) agar muat teks panjang
    ...
    def draw_control_guide(self, surface):
        w, h = 180, 140; x = SCREEN_WIDTH - w - 10; y = 10 
    ...
    def draw_main_menu(self):
        if ASSETS["menu_frames"]:
            ...
            if self.menu_frame_index >= len(ASSETS["menu_frames"]) - 1:
                self.menu_frame_index = len(ASSETS["menu_frames"]) - 1; self.menu_anim_dir = -1 
            elif self.menu_frame_index <= 0:
                self.menu_frame_index = 0; self.menu_anim_dir = 1 
            frame_to_draw = ASSETS["menu_frames"][int(self.menu_frame_index)]
            SCREEN.blit(frame_to_draw, (0, 0))
        else: SCREEN.fill(C_BG_DARK)
        ...
        for p in self.menu_particles: p.update(); p.draw(SCREEN)

        scale = 1.0 + 0.05 * math.sin(pygame.time.get_ticks() * 0.003)
        title_surf = FONT_TITLE.render("HARVEST ARENA", True, C_ACCENT_GOLD)
        w = int(title_surf.get_width() * scale); h = int(title_surf.get_height() * scale)
        ...
        self.btn_new.draw(SCREEN); self.btn_load.draw(SCREEN)
        self.btn_sets.draw(SCREEN); self.btn_quit.draw(SCREEN)
    ...
    def draw_pause_menu(self):
        ...
        self.btn_res.draw(SCREEN); self.btn_sav.draw(SCREEN)
        self.btn_lod.draw(SCREEN); self.btn_men.draw(SCREEN)
    ...
    def draw_shop(self):
        ...
        for cat in categories:
            ...
            cat_rects.append((r, cat)); start_x += 200
        items_to_show = SHOP_INVENTORY[self.shop_category]; y_pos = 220; x_pos = 100
        for name, buy_price in items_to_show:
            ...
            item_data = ITEM_DATA.get(name, {"sell": 0}); sell_price = item_data["sell"]
            ...
            y_pos += 70
            if y_pos > 480: y_pos = 220; x_pos += 400
    ...
    def draw_inventory_menu(self):
        ...
        for slot_code in self.hotbar.slots:
            if slot_code in CODE_TO_NAME: equipped_items.add(CODE_TO_NAME[slot_code])
        y_pos = 150; x_pos = 100
        mx, my = get_corrected_mouse()
        for item, count in GAME_STATS.inventory.items():
            if item in equipped_items or count <= 0: continue
            ...
            y_pos += 70
            if y_pos > 400: y_pos = 150; x_pos += 260
    ...
    def update_main_menu(self, event):
        ...
        if self.btn_new.update(events): 
            ...
            self.selected_save_idx = 0
            AUDIO.play("select")
        elif self.btn_sets.update(events): self.state = STATE_SETTINGS
        elif self.btn_quit.update(events): pygame.quit(); sys.exit()
    ...
    def update_settings_menu(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if mx < SCREEN_WIDTH//2: 
                if 250 < my < 280: AUDIO.set_music_volume(AUDIO.music_vol - 0.1)
                elif 350 < my < 380: AUDIO.set_sfx_volume(AUDIO.sfx_vol - 0.1)
            else: 
                if 250 < my < 280: AUDIO.set_music_volume(AUDIO.music_vol + 0.1)
                elif 350 < my < 380: AUDIO.set_sfx_volume(AUDIO.sfx_vol + 0.1)
    ...
    def run(self):
        ...
        while running:
            clock.tick(60); self.clean_hotbar()
            ...
            if self.fade_state == "FADE_OUT":
                ...
                if self.black_screen_timer <= 0:
                    if self.pending_menu_action == "NEW":
                        GAME_STATS.reset(); self.world.reset_world()
                ...
                self.fade_alpha -= 10
                if self.fade_alpha <= 0: self.fade_alpha = 0; self.fade_state = "IDLE"

            # --- LOGIKA GAME ---
            if self.state == STATE_OVERWORLD and self.fade_state == "IDLE":
                self.game_time += self.time_speed
                if self.game_time >= 26 * 60: self.trigger_day_transition(passed_out=True) 
                self.hunger_timer += 1
                if self.hunger_timer >= 200: 
                    self.hunger_timer = 0; GAME_STATS.hunger = max(0, GAME_STATS.hunger - 1)
                    if GAME_STATS.hunger <= 0: GAME_STATS.hp = max(0, GAME_STATS.hp - 1)
                    if GAME_STATS.hp <= 0: self.trigger_day_transition(passed_out=True) 

            target_cam_x = self.player.global_x - SCREEN_WIDTH // 2; target_cam_y = self.player.global_y - SCREEN_HEIGHT // 2
            ...
            if self.state == STATE_OVERWORLD:
                for t in self.world.get_all_tiles(): t.update()
            ...
            for event in pygame.event.get():
                ...
                if self.state == STATE_MAIN_MENU: self.update_main_menu(event)
                elif self.state == STATE_SETTINGS: self.update_settings_menu(event)
                elif self.state == STATE_PROLOGUE: self.update_prologue(event) 
                    ...
                    events = [event]
                    if self.btn_res.update(events): self.state = STATE_OVERWORLD
                    elif self.btn_sav.update(events): self.save_game()
                    elif self.btn_lod.update(events): 
                        self.state = STATE_LOAD_MENU
                        self.save_files = self.get_save_files()
                        self.selected_save_idx = 0
                    elif self.btn_men.update(events): self.state = STATE_MAIN_MENU; self.play_menu_music()
                    if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE: self.state = STATE_OVERWORLD

                elif self.state == STATE_OVERWORLD and self.fade_state == "IDLE":
                    if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE: self.state = STATE_PAUSE
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if event.button == 4: self.hotbar.selected = (self.hotbar.selected - 1) % len(self.hotbar.slots); AUDIO.play("select")
                        elif event.button == 5: self.hotbar.selected = (self.hotbar.selected + 1) % len(self.hotbar.slots); AUDIO.play("select")
                        ...
                        # --- [FITUR UNEQUIP] KLIK KANAN UNTUK KEMBALIKAN KE INVENTORY ---
                            ...
                            for i in range(len(self.hotbar.slots)):
                                r = pygame.Rect(start_x + (i * 55), SCREEN_HEIGHT - 70, 50, 50)
                                if r.collidepoint((mx, my)):
                                    self.hotbar.selected = i; AUDIO.play("select"); clicked_hotbar = True; break
                            
                            if not clicked_hotbar:
                                if not self.try_sleep():
                                    tool = self.hotbar.slots[self.hotbar.selected]; t = self.get_tile_under_mouse(camera_x, camera_y)
                    ...
                    if event.type == pygame.KEYDOWN:
                        ...
                        elif event.key == pygame.K_q: self.state = STATE_SHOP; AUDIO.play("select")
                        elif event.key == pygame.K_e: self.state = STATE_INVENTORY; AUDIO.play("select")
                    ...
                    if event.type == pygame.KEYDOWN and (event.key == pygame.K_q or event.key == pygame.K_ESCAPE): self.state = STATE_OVERWORLD
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        mx, my = get_corrected_mouse()
                        start_x = 100
                        for cat in ["SEEDS", "ITEMS", "LOOT", "CROPS"]:
                            if pygame.Rect(start_x, 140, 180, 40).collidepoint((mx, my)): self.shop_category = cat; AUDIO.play("select")
                            start_x += 200
                        items = SHOP_INVENTORY[self.shop_category]; y_p = 220; x_p = 100
                        for name, price in items:
                            r = pygame.Rect(x_p, y_p, 380, 60)
                            if r.collidepoint((mx, my)):
                                if event.button == 1: # Beli
                                    if GAME_STATS.gold >= price: GAME_STATS.gold -= price; GAME_STATS.inventory[name] = GAME_STATS.inventory.get(name,0)+1; AUDIO.play("buy")
                                    else: AUDIO.play("error")
                                elif event.button == 3: # Jual
                                    if GAME_STATS.inventory.get(name, 0) > 0:
                                        sv = ITEM_DATA.get(name, {}).get("sell", 0)
                                        if sv > 0: GAME_STATS.inventory[name] -= 1; GAME_STATS.gold += sv; AUDIO.play("sell")
                                    else: AUDIO.play("error")
                            y_p += 70
                            if y_p > 480: y_p = 220; x_p += 400

                elif self.state == STATE_INVENTORY:
                    if event.type == pygame.KEYDOWN and (event.key == pygame.K_e or event.key == pygame.K_ESCAPE): self.state = STATE_OVERWORLD
                        ...
                        if event.button == 3: 
                            ...
                            for item, count in GAME_STATS.inventory.items():
                                ...
                                if pygame.Rect(it_x, it_y, 250, 60).collidepoint((mx, my)):
                                    if item in NAME_TO_CODE: self.selected_item_name = item; AUDIO.play("select")
                                it_y += 70
                                if it_y > 400: it_y = 150; it_x += 260
                            if self.selected_item_name:
                                ...
                                for i in range(len(self.hotbar.slots)):
                                    r = pygame.Rect(start_x + (i * 55), 500, 50, 50)
                                    if r.collidepoint((mx, my)):
                                        self.hotbar.slots[i] = NAME_TO_CODE[self.selected_item_name]; self.selected_item_name = None; AUDIO.play("buy")
            ...
            if self.state == STATE_MAIN_MENU: self.draw_main_menu()
            elif self.state == STATE_SETTINGS: self.draw_settings_menu()
            elif self.state == STATE_PROLOGUE: self.draw_prologue()
            elif self.state == STATE_INPUT_NAME: self.draw_input_name_menu() 
            elif self.state == STATE_LOAD_MENU: self.draw_load_select_menu() 
            
            elif self.state == STATE_OVERWORLD or self.state == STATE_PAUSE:
                self.player.update(pygame.key.get_pressed(), [self.house_rect]) 
                self.check_encounter(); self.enemies_ow.update(self.safe_zone_rect) 
                ...
                for e in self.enemies_ow:
                    esx = e.global_x - camera_x - 20; esy = e.global_y - camera_y - 20
                ...
                sx = self.house_rect.x - camera_x; sy = self.house_rect.y - camera_y
                render_list.append({
                    "type": "HOUSE",
                    "img": ASSETS["house"],
                    "pos": (sx, sy),
                ...
                if t_t: pygame.draw.rect(SCREEN, (255, 255, 255), (t_t.rect.x-camera_x, t_t.rect.y-camera_y, TILE_SIZE, TILE_SIZE), 2)
                
                self.draw_time_filter(); self.hotbar.draw(SCREEN); self.draw_hud(); self.draw_control_guide(SCREEN)
                ...
                if self.state == STATE_PAUSE: self.draw_pause_menu()
                ...
                self.battle.update(); self.battle.draw(SCREEN)
                if not self.battle.active:
                    if self.battle.won: self.state = STATE_OVERWORLD
                    else:
                        GAME_STATS.gold = max(0, GAME_STATS.gold // 2); GAME_STATS.hp = 10; self.show_msg("Defeated...")
                        self.player.global_x = (MAP_WIDTH//2)*TILE_SIZE; self.player.global_y = ((MAP_HEIGHT//2)*TILE_SIZE)+150; self.state = STATE_OVERWORLD
                    self.spawn_enemies(20)

            if self.fade_state != "IDLE":
                fs = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT)); fs.fill((0,0,0)); fs.set_alpha(self.fade_alpha); SCREEN.blit(fs, (0,0))
